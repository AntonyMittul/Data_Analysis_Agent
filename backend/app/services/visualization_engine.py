...
def build_kpi_cards(df: pd.DataFrame, quality: dict | None = None):
    ...
    # 2) Top Categorical (e.g. Top Region)
    if categoricals:
        col = categoricals[0]
    ...
    # 4) Fallback to another categorical if we still need a 4th card
    if len(cards) < 4 and len(categoricals) > 1:
        col = categoricals[1]
        vc = clean[col].value_counts()
        if len(vc):
            cards.append({
